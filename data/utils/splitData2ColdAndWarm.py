...
parser.add_argument('--csvfile_sep', type=str, default=',')
...
origin_warm_emb = df.loc[warm_emb_idx]
origin_warm_emb.to_csv(os.path.join(store_path, 'origin_warm.csv'))
...
df_warm_val.to_csv(os.path.join(store_path, 'origin_warm_val.csv'))
idx_to_move = df_warm_val[True ^ df_warm_val['user'].isin(warm_emb_user_set)].index
print(f'warm val idx_to_move is {idx_to_move}')
...
print("Warm emb splitting finished: {} -> {}".format(org_warm_emb_len, len(warm_emb_idx)))
...
print("cold: {} -> {}".format(org_cold_len, len(cold_idx)))
