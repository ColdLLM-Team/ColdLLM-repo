...
def read_convert_dict_file(datadir, dataset):
    import pickle
    with open(datadir + dataset + '/convert_dict.pkl', 'rb') as f:
        convert_dict = pickle.load(f)
    return convert_dict

def save_top_csv_file(cold_item_set_dict, csv_file_name):
...
def set_Logger(logpath, logfilename, logger):
    import logging
    ...
    logfile = logpath + logfilename + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '.log'
