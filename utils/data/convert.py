...
"""Get testing users"""
emb_user = np.array(list(set(df_emb['user'])), dtype=np.int32)  # warm_emb.csv 就是训练集中所有的item
warm_val_user = np.array(list(set(df_warm_val['user'])), dtype=np.int32) # warm_val.csv
warm_test_user = np.array(list(set(df_warm_test['user'])), dtype=np.int32) # warm_test.csv
cold_val_user = np.array(list(set(df_cold_val['user'])), dtype=np.int32) # cold_val.csv
cold_test_user = np.array(list(set(df_cold_test['user'])), dtype=np.int32) # cold_test.csv
...
emb_item_nb = utils.df_get_neighbors(df_emb, 'item', item_num)  # index [item_array] # 这里其实是warm_train中的交互列表
warm_val_item_nb = utils.df_get_neighbors(df_warm_val, 'item', item_num)
warm_test_item_nb = utils.df_get_neighbors(df_warm_test, 'item', item_num)
cold_val_item_nb = utils.df_get_neighbors(df_cold_val, 'item', item_num)
cold_test_item_nb = utils.df_get_neighbors(df_cold_test, 'item', item_num)
pos_item_nb = utils.df_get_neighbors(df_pos, 'item', item_num)
overall_val_item_nb = utils.df_get_neighbors(df_overall_val, 'item', item_num)
overall_test_item_nb = utils.df_get_neighbors(df_overall_test, 'item', item_num)
...
para_dict['warm_val_item_nb'] = warm_val_item_nb
para_dict['warm_test_item_nb'] = warm_test_item_nb
para_dict['cold_val_item_nb'] = cold_val_item_nb
para_dict['cold_test_item_nb'] = cold_test_item_nb
para_dict['overall_val_item_nb'] = overall_val_item_nb
para_dict['overall_test_item_nb'] = overall_test_item_nb
